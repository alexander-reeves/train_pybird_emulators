...
def get_pybird_chi2(knots, kk, cov_inv, pk, dlogpk_r, pk_max, N, N_rec):
    ...
    N.compute({'kk': kk, 'pk_lin': jnp.array(pk), 'D': D1, 'f': f1, 'z': z, 'Omega0_m': Omega0_m},
            do_core=True, do_survey_specific=True)
    
    bpk_truth = N.get(eft_params)*pk_max

    N_rec.compute({'kk': kk, 'pk_lin': jnp.array(prediction_pklin), 'D': D1, 'f': f1, 'z': z, 'Omega0_m': Omega0_m},
            do_core=True, do_survey_specific=True)
...
def minimize_spline_all(knots_start, pk_info_dict_list, kk, k_l, k_r, cov_inv):
    ...
    def get_chi2_vectorized(internal_knots):
        knots = jnp.concatenate((jnp.array([leftmost_knot]), internal_knots, jnp.array([rightmost_knot])))
    ...
    result = optimize.minimize(get_chi2_vectorized_jit, internal_knots_start, method=method, options={'disp': True})
...
def main(indices, args):
    ...
    for index in indices: 
        ...
        def scale_knots_except_bounds(knots, scale_percentage=5):
            ...
            scaled_knots = np.copy(knots)  # Make a copy to preserve the original knots array
        ...
        knots_start = scale_knots_except_bounds(knots, scale_percentage=scale_percentage)
        ...
        while delta_chi2 > threshold:
            knots_i, minchi2_i = minimize_spline_all(knots, pk_info_dict_list, kk, k_l, k_r, cov_inv)
            ti = time.time()
            print ('step %s in %.1f sec, [red min chi2]^0.5=%.10f' % (i, ti-t0, minchi2_i**.5))
            delta_chi2 = abs(minchi2 - minchi2_i)
            minchi2 = 1. * minchi2_i
            t0 = 1.*ti
            i += 1

        np.save(f"./optimised_knots_lss_err/knots_temp_file_knots_{nknots}_index_{index}.npy", knots)
