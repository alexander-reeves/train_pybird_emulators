...
def setup(args):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="if true, will perform IR resummation",
    )
    ...
    with h5py.File(out_fn, "w") as hdf_file:
        for index in indices:
            ...
            for i in range(index * num_per_ind, (index + 1) * (num_per_ind)):
                ...
                if resum:
                    npz_file_path = (
                        output_fn + f"/testing_data_bpk_{nknots}_knots{i}.npz"
                    )

                else:
                    npz_file_path = (
                        output_fn + f"/testing_data_bpk_{nknots}_knots{i}_noresum.npz"
                    )
    ...
    for index in indices:
        for i in range(index * num_per_ind, (index + 1) * (num_per_ind)):
            if i < 5000:
                ...
                bpk_true, bpk_emu = utils.compute_1loop_bpk_test(
                    cosmo_dict, kk, eft_params, knots, resum=resum
                )

                if resum:
                    np.savez(
                        output_fn + f"/testing_data_bpk_{nknots}_knots{i}",
                        bpk_true=bpk_true,
                        bpk_emu=bpk_emu,
                    )
                else:
                    np.savez(
                        output_fn + f"/testing_data_bpk_{nknots}_knots{i}_noresum",
                        bpk_true=bpk_true,
                        bpk_emu=bpk_emu,
                    )
