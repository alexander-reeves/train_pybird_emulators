...
def _get_spline(logknots, logpk, kk, bc_type=None):
    if bc_type:
        ilogpk_spline = make_interp_spline(
            logknots, logpk, k=3, bc_type=([(1, 1)], [(1, bc_type[1][0][1])])
        )
    else:
        ilogpk_spline = make_interp_spline(logknots, logpk, k=1)
...
def minimize_spline_all(knots_start, lhc_pk_dict, kk_, k_l, k_r, what="ploop"):
    ...
    def get_chi2(internal_knots):
        ...
        for d in lhc_pk_dict:
            chi2 += np.sum(
                (
                    scaling
                    * (d["pkk_"] - get_spline(knots, d["ilogpk"], kk_, d["bc_type"]))
                    / d["err"]
                )
                ** 2
            )
    ...
    m = minimize(
        get_chi2, internal_knots_start, method=method, bounds=nk * [(k_l, k_r)]
...
def setup(args):
    ...
    nbar = (
        2e-2  # ideal nbar (for b1~2) (in [Mpc/h]^3), > 12 x better than DESI / Euclid
    )
    cov = utils.get_cov(kk, ipk_h, 1.0, 0, mult=1, nbar=nbar, Vs=Vs)
    err = np.sqrt(np.diag(cov))

    lhc_cosmo_dicts, lhc_pk_lin, nbank, indices_pkbank = load_pk_cosmo_dict(
        "../lhc_bank_z0p5to2p5/", nbank=nbank, kk=kk
    )
    lhc_pk_dict = []

    for i in range(indices_pkbank.shape[0]):

        pk_i = 1.0 * lhc_pk_lin[i]
        pk_max = np.max(pk_i)
...
def main(indices, args):
    ...
    for index in indices:
        ...
        perturb_scale = 0.03
        knots_lin += np.random.uniform(
            -perturb_scale * knots_lin, perturb_scale * knots_lin, size=knots_lin.shape
        )
        knots_log_low_k[1:] *= np.exp(
            np.random.uniform(
                -perturb_scale, perturb_scale, size=knots_log_low_k[1:].shape
            )
        )
        knots_log[:-1] *= np.exp(
            np.random.uniform(-perturb_scale, perturb_scale, size=knots_log[:-1].shape)
        )

        knots_start = np.concatenate(
            ([k_l], knots_log_low_k[1:], knots_lin, knots_log[:-1], [k_r])
        )
        ...
        while delta_chi2 > 0.00001:
            knots, minchi2_i = minimize_spline_all(
                knots, lhc_pk_dict, kk_, k_l, k_r, what="ploop"
            )
            ti = time()
            print(
                "step %s in %.1f sec, [red min chi2]^0.5=%.3f"
        ...
        np.save(
            f"./optimised_knots_cov_err/knots_temp_file_knots_{nknots}_index_{index}.npy",
            knots,
        )
