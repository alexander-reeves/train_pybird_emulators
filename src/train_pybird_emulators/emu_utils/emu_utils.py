...
import os
from train_pybird_emulators.emu_utils.interpolated_univariate_spline_jax import InterpolatedUnivariateSpline_jax
...
def get_pgg_from_params(params, parameters_dicts, N, kk, knots):
    ...
    f = input_dict["f"]
    #get the logpk_knots
    logpk_knots = [input_dict[key] for key in sorted(input_dict.keys(), key=lambda x: int(x.split('_')[-1]) if x.startswith('pk_knot_') else float('inf')) if key.startswith('pk_knot_')]
    ...
    P11l, Pctl, Ploopl, IRPs11, \
            IRPsct, IRPsloop = get_pgg_from_linps_and_f_and_A(pk_lin, kk, N, f, pkmax)
...
def get_green_function_from_params(params, parameters_dicts):
    ...
    GF = GreenFunction(Omega0_m=input_dict["omega_m"], w=input_dict["w0"], quintessence=True)
    a = 1/(1+input_dict["z"])
    Y1 = GF.Y(a)
    G1t = GF.mG1t(a)
    V12t = GF.mV12t(a)
...
def get_training_data_from_hdf5(fn, piece_name, ntrain, mono, quad_hex):
    ...
    with h5py.File(fn, 'r') as f:
        LOGGER.info(f"total number of available training points: {f['params'].shape[0]}")
        ...
        if piece_name is not None:
            ...
            y_train = np.vstack([f[key][:ntrain] for key in f.keys() if key != 'params']).T
